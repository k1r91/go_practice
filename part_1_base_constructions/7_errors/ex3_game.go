...
func (ise invalidStepError) Error() string {
	return fmt.Sprintf("(from custom error) cannot %v", ise.step)
}
...
func (neo notEnoughObjectsError) Error() string {
	return fmt.Sprintf("(from custom error) there are no %ss left", neo.obj)
}
...
func (cle commandLimitExceededError) Error() string {
	return fmt.Sprintf("(from custom error) you don't want to %s anymore", cle.desc)
}
...
func (ole objectLimitExceededError) Error() string {
	return fmt.Sprintf("(from custom error) you already have a %s", ole.obj)
}
...
func (goe gameOverError) Error() string {
	return fmt.Sprintf("game over at step %d, error: %v", goe.nSteps, goe.err)
}
...
func (p *player) do(cmd command, obj thing) error {
	...
	case eat:
		if p.nEaten > 1 {
			return commandLimitExceededError{desc: "you don't want to eat anymore"}
		}
		p.nEaten++
	case take:
		if p.has(obj) {
	...
	case talk:
		if p.nDialogs > 0 {
			return commandLimitExceededError{desc: "you don't want to talk anymore"}
		}
		p.nDialogs++
	}
	return nil
...
func giveAdvice(err error) string {
	// ...
	var objectErr objectLimitExceededError
	var limitErr notEnoughObjectsError
	var actionErr invalidStepError
	var commandLimitErr commandLimitExceededError
	if errors.As(err, &objectErr) {
		return fmt.Sprintf("don't be greedy, 1 %s is enough", objectErr.obj)
	} else if errors.As(err, &limitErr) {
		return fmt.Sprintf("be careful with scarce %s", limitErr.obj.name)
	} else if errors.As(err, &actionErr) {
		return fmt.Sprintf("things like '%v' are impossible", actionErr.step)
	} else if errors.As(err, &commandLimitErr) {
		switch commandLimitErr.desc {
		case "you don't want to talk anymore":
			return fmt.Sprintf("talk to less")
		case "you don't want to eat anymore":
